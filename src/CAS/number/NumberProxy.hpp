...
public:
    ...
    NumberProxy(const Number& aNumber) { number = aNumber.clone(); }
    NumberProxy(Number* aNumber)       { number = aNumber;         }
    ...
    virtual ~NumberProxy() { delete number; }

    virtual bool isValid(void) const { return (number != NULL); }
