...
Expression::Ptr Builder::operator()(const string& name, const vector<Expression::Ptr>& children) const
{
    if (name == "+")
    {
        if (children.size() == 0)
            throw invalid_argument("children.size() invalid in DS::CAS::Expressions::Builders::Builder::operator()");
        return add(children);
    }
    if (name == "-")
    {
    ...
    if (name == "")
        throw invalid_argument("name == empty in DS::CAS::Expressions::Builders::Builder::operator()");
    return symbol(name, children);
}
Expression::Ptr Builder::operator()(const string& name) const
{
    if (name == "")
...
Expression::Ptr Builder::operator()(const string& name, Expression::Ptr ptr1, Expression::Ptr ptr2) const
{
